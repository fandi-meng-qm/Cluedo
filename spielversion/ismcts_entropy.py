...
class ISMCTSBot(pyspiel.Bot):
  ...
  def get_final_policy(self, state, node):
    ...
    #             for action, child in node.child_info.items()]
    if self._final_policy_type == ISMCTSFinalPolicyType.MAX_VALUE:
      assert node.total_visits > 0
      max_value = -float('inf')
      count = 0
      for action, child in node.child_info.items():
  ...
  def initstate_from_infostate(self,state):
    ...
    if init_state.cards_dice[state.params.n_players]&{0,1,2,3,4,5} == set():
      init_state.cards_dice[state.params.n_players].add(random.sample(cards_left & {0, 1, 2, 3, 4, 5}, 1)[0])
    if init_state.cards_dice[state.params.n_players]&{6,7,8,9,10,11} == set():
      init_state.cards_dice[state.params.n_players].add(random.sample(cards_left & {6, 7, 8, 9, 10, 11}, 1)[0])
    if init_state.cards_dice[state.params.n_players]&{12,13,14,15,16,17,18,19,20} == set():
